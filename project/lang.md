...
val =
  ...
  | FinA of rsm                  // Для пересечения запросов с графами
...
## Пример скрипта
```
let g' = load graph "wine" // загрузка графа wine
let g' = load graph from 'home/wine.dot' // загрузка графа из файла

let g = set start of (set finals of g' to (vertices of g')) to {0..100}
//устанавливаем все вершины финальными, а вершины с 0 по 100 стартовыми

let l1 = "l1" | "l2" // создание запроса (или)

let q1 = ("type" | l1)** // создание запроса (или + звезда клини)
let q2 = "sub_class_of" .. l1 создание запроса (конкатенация)

let res1 = g & q1 // выполнение запроса 1
let res2 = g & q2 // выполнение запроса 2

print res1 // печать результата (всего объекта)

let s = start set of g // получение стартовых вершин

let vertices1 = filter (fun v -> v in s) (map (fun ((u_g,u_q1),l,(v_g,v_q1)) -> u_g) (get edges of res1))
//получить стартовые вершины графа являющимися началами каких-то путей
